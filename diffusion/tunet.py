...
class TimeConvAttentionBlock(nn.Module):
    def __init__(
        ...
        self.skip_gate = nn.Sequential(
            nn.Conv2d(in_chans, 1, kernel_size=1), nn.Sigmoid()
        )
    ...
    def forward(
        ...
        # Apply gate and skip connection
        ...
        # t_emb shape: (B, time_emb_dim) -> (B, out_chans * 2) -> shift, bias
        # x shape: (B, out_chans, W, H)
        t_emb = self.time_mlp(t_emb)[:, :, None, None]
        shift, bias = t_emb.chunk(2, dim=1)
        x = x * (1 + shift) + bias  # Broadcasting shift and bias

        # Conv2d + GroupNorm + SiLU
        x = self.layer2(x)

        # Squeeze-and-Excitation block
        # se_weight shape: (B, out_chans, 1, 1)
