...
from callback import EarlyStopping, CheckpointResume, ModelCheckpoint, MetricsCallback
...
class Trainer:
    ...
    def _load_data(self):
        ...
        val_ds = DataSet(
            root=data_cfg["val_path"], split="val", image_size=self.config["image_size"]
        )
    ...
    def _setup_loss(self):
        ...
        if hasattr(nn, loss_cfg["type"]):
            ...
            raise ValueError(f"Unknown loss function: {loss_cfg['type']}")
        ...
        for i, (x, y) in enumerate(self.train_loader, 1):
            ...
            for cb in self.callbacks:
                if hasattr(cb, "on_train_batch_end"):
                    cb.on_train_batch_end(self, preds, y)
    ...
    def run(self):
        ...
        # Add metrics callback for segmentation evaluation
        num_classes = self.config["model"]["params"]["n_classes"]
        class_names = ["Background", "Unknown", "Foreground"]  # For trimap segmentation
        self.callbacks.append(MetricsCallback(num_classes, class_names))
