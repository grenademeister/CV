...
def unpack_cifar(direc):
    ...
    for filename in os.listdir(direc):
        if filename[:5] == "data_":
            ...
            y += data[b"labels"]
    assert X, (
        "No data was found in '%s'. Are you sure the CIFAR10 data is there?" % direc
    )
...
def unpack_mnist(direc):
    ...
    try:
        ...
        print(
            "Make sure that you have downloaded the data and put in %s\n Also make sure that the spelling is correct. \
              the MNIST data comes in t10k-images.idx3-ubyte or t10k-images-idx3-ubyte. We expect the latter"
            % (direc)
        )
...
class Datagen:
    ...
    def __init__(self, direc_mnist, direc_cifar):
        ...
        self.data = {
            "mnist": {"train": None, "val": None},
            "cifar": {"train": None, "val": None},
        }
    ...
    def sample(self, batch_size, norm=True, dataset="train"):
        ...
        for i in range(batch_size):
            im_cifar[
                i,
                width_start[i] : width_start[i] + size_mnist,
                height_start[i] : height_start[i] + size_mnist,
            ] += mnist_batch[i]
            segm_maps[
                i,
                width_start[i] : width_start[i] + size_mnist,
        ...
        if norm:
            im_cifar = (im_cifar - 130.0) / 70.0
        return im_cifar, segm_maps


def save_data(data, segm_map, batch_size):
    # Create directory if it doesn't exist
    os.makedirs("data/synth", exist_ok=True)

    for i in range(batch_size):
        print(f"\r{i}/{batch_size} done", end="")
        # Handle normalized data - reverse the normalization
        # Original normalization: (im_cifar - 130.0) / 70.0
        # Reverse: data * 70.0 + 130.0
        img_data_array = (data[i] * 70.0 + 130.0).astype(np.uint8)
        img_data_array = np.clip(img_data_array, 0, 255)

        # Segmentation map - convert to 0-255 range
        img_segm_array = (segm_map[i] * 255).astype(np.uint8)

        # Create PIL Images
        img_data = Image.fromarray(img_data_array)
        img_segm = Image.fromarray(img_segm_array, mode="L")  # 'L' for grayscale

        # Save images
        img_data.save(f"data/synth/data_{i}.png")
        img_segm.save(f"data/synth/segm_{i}.png")


if __name__ == "__main__":
    dg = Datagen("data/mnist", "data/cifar")
    data, segm_maps = dg.sample(2048)
    print(segm_maps[1].shape)
    save_data(data, segm_maps, 2048)
    # plot_data(data)
